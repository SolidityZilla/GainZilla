...
### Contract
IUniswapV2Router : contracts/gainzilla.sol
...
### Contract
MemeCoin : contracts/gainzilla.sol
...
```solidity
function setTaxRates(uint16 buyLp, uint16 buyEco, uint16 sellLp, uint16 sellEco) external
```
...
```solidity
function configureCooldown(bool enabled, uint32 duration) external
```

### configureDiamondFloor

```solidity
function configureDiamondFloor(bool enabled, uint16 triggerPercent) external
```
